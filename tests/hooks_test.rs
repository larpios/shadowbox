...
fn test_hooks_install_and_uninstall() {
    ...
    assert!(output.status.success(), "hooks install should succeed. stderr: {}", String::from_utf8_lossy(&output.stderr));
    ...
    for hook_name in &["post-checkout", "post-merge"] {
        ...
        #[cfg(unix)]
        {
            ...
            assert!(permissions.mode() & 0o111 != 0, "{} should be executable", hook_name);
        }
    }
    ...
    assert!(output.status.success(), "hooks uninstall should succeed. stderr: {}", String::from_utf8_lossy(&output.stderr));
