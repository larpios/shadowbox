...
fn test_track_subcommand() {
    ...
    assert!(shadowbox_content.lines().any(|l| l == "test_track_file_1.txt"));
    ...
    let count = shadowbox_content2.lines().filter(|l| l == &"test_track_file_1.txt").count();
...
fn test_path_normalization() {
    ...
    let shadowbox_content = fs::read_to_string(shadowbox).unwrap_or_default();
    assert!(shadowbox_content.lines().any(|l| l == "test_subdir_norm_2/test_file.txt"));
...
fn test_append_without_newline() {
    ...
    assert!(content.contains("\ninitial_line\n") || content.starts_with("initial_line\n"), "Should have newline after initial_line, got: {:?}", content);
    assert!(content.contains("test_file_bug_3.txt\n"), "Should have track line, got: {:?}", content);
