...
fn test_distributed_push_pull() {
    ...
    Command::new("git").args(["init", "--bare"]).current_dir(project_remote_dir.path()).status().unwrap();
    ...
    // 2. Setup Project A
    Command::new("git").args(["init"]).current_dir(project_dir_a.path()).status().unwrap();
    Command::new("git").args(["remote", "add", "origin", &project_remote_dir.path().to_string_lossy()]).current_dir(project_dir_a.path()).status().unwrap();
    
    // 3. Configure Shadowbox
    run_shadowbox(vec!["store", "add", "my_store", &remote_store_dir.path().to_string_lossy()], project_dir_a.path());
    ...
    Command::new("git").args(["init"]).current_dir(project_dir_b.path()).status().unwrap();
    Command::new("git").args(["remote", "add", "origin", &project_remote_dir.path().to_string_lossy()]).current_dir(project_dir_b.path()).status().unwrap();
    
    // 6. Pull in Project B
    let output = run_shadowbox(vec!["pull"], project_dir_b.path());
    assert!(output.status.success(), "Pull failed: {}", String::from_utf8_lossy(&output.stderr));

    // 7. Verify file restored in B
    assert!(project_dir_b.path().join(test_file).exists());
    assert_eq!(fs::read_to_string(project_dir_b.path().join(test_file)).unwrap(), "secret content");
