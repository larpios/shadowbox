...
"#,
    ...
    assert!(
        attributes.contains(large_file),
        "Large file should be tracked in .gitattributes"
    );
    assert!(
        attributes.contains("filter=lfs"),
        "LFS filter should be active"
    );
    assert!(
        !attributes.contains(small_file),
        "Small file should NOT be tracked in LFS"
    );
}
